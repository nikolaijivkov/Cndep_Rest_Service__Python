...
class Conflict(RequestError):
    ...
    def __init__(self, message, hide_traceback=True):
        RequestError.__init__(self,message)
        self.hide_traceback = hide_traceback
...
def handle_request(environ, start_response):
    ...
    try:
        ...
        return handle_error(e, request) #,True
...
def handle_error(exception, request=None, mute=False):
    ...
    if not getattr(exception, 'hide_traceback', False):
        ...
        if(mute): 
            print "%s occurred on :'%s', MUTED" % (str(exception.__class__).replace('exceptions.', '') ,request._environ['PATH_INFO'])
        else: 
...
def forbidden(request, exception):
    #'Forbidden'
    response = Response('Forbidden: '+str(exception), status=403, content_type='text/plain')
...
def not_found(request, exception):
    #'Not Found'
    response = Response('Not Found: '+str(exception), status=404, content_type='text/plain')
    return response.send(request._start_response)

@error(409)
def conflict(request, exception):
    #'Conflict'
    response = Response('Conflict: '+str(exception), status=409, content_type='text/plain')
    return response.send(request._start_response)

@error(500)
def app_error(request, exception):
    #'Application Error'
    response = Response('Application Error: '+str(exception), status=500, content_type='text/plain')
    return response.send(request._start_response)
